...
- [ ] 正しい ID / PW でログインすると JWT が返る
- [ ] 誤った認証情報では `401` が返る
- [ ] JWT の有効期限は設定可能（デフォルト 24 時間）
- [ ] `POST /api/v1/rounds` を JWT なしで呼ぶと `401` が返る
- [ ] `GET /api/v1/analysis` は JWT なしで `200` が返る
- [ ] JWT の秘密鍵は環境変数 `JWT_SECRET` で管理する
