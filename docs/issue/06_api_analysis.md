...
- `history` は 18 件ウィンドウごとの確率値履歴を表す
- `total_rounds < 18` の間は `ready: false` を返し、`history` は返さない
- `total_rounds = 18` のとき `history` は 1 件
- 19 件目以降はラウンド追加ごとに 1 件ずつ増加する
- 件数は `max(0, min(total_rounds - 17, 72))`
- 72 件を超える場合は直近 72 ウィンドウ分のみ返す

## 受け入れ条件

- [x] JWT なしで `200` が返る
- [x] `ready: false` のとき統計値・グラフデータは含まれない
- [x] `chart_data` が最大 72 件で返る
- [x] Redis キャッシュが有効なとき DB クエリが発生しない
- [x] 倍率登録・リセット後にキャッシュが invalidate されて新値が返る
