...
- [x] 条件① 低倍率連続が L 件以上で `no_entry.active = true` になる
- [x] 条件② 高倍率直後の急収束が検出される
- [x] 条件③ CV < 0.25 で `no_entry.active = true` になる
- [x] 複数条件が `reasons` にすべて列挙される
- [x] `ready: false` の場合 `no_entry` フィールドが含まれない
- [x] 分析ページにカラーインジケーターと条件説明が表示される
- [x] 単体テストが存在する（各条件の境界値を含む）
